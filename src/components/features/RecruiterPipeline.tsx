...
import { ShieldCheck, Search, Filter, Calendar, ExternalLink, CheckCircle, Award } from 'lucide-react';
    ...
    <div className="space-y-8">
      ...
      <div className="space-y-4">
        <h3 className="text-lg font-bold text-slate-900 dark:text-white flex items-center justify-between">
          <span>Verified Candidates Matches ({filteredCandidates.length})</span>
              ...
              <Card key={cand.id} hoverable className="p-6 space-y-4 border-slate-200 dark:border-border-default">
                <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                  ...
                  <div className="flex items-start space-x-4">
                    <div className="w-12 h-12 rounded-2xl bg-purple-600 text-white font-extrabold flex items-center justify-center text-lg shadow-md">
                      {cand.avatar}
                    </div>
      ...
      {comparingCandidates.length === 2 && (
        <Card className="p-6 space-y-4 bg-purple-50 dark:bg-purple-950/20 border-purple-200 dark:border-purple-800/40">
          <div className="flex items-center justify-between border-b border-purple-200 dark:border-purple-800/40 pb-3">
            <h3 className="font-extrabold text-slate-900 dark:text-white text-base flex items-center">
              ...
              Side-by-Side Candidate Proof Comparison
            </h3>
            <button
          ...
          <div className="grid grid-cols-2 gap-6 text-xs">
            {comparingCandidates.map((c) => (
              <div key={c.id} className="p-4 bg-white dark:bg-bg-base rounded-xl border border-purple-200 dark:border-purple-900 space-y-2">
                <div className="font-bold text-slate-900 dark:text-white text-sm flex justify-between">
                  <span>{c.name}</span>
                  <span className="text-purple-600">{c.proofScore}% Score</span>
                </div>
                <p className="text-slate-600 dark:text-gray-300">{c.headline}</p>
                <div className="text-emerald-600 font-mono font-semibold pt-1">
                  {c.liveAppsCount} Production Live Apps
                </div>
                <div className="text-slate-500 font-mono text-[11px]">
                  {c.verifiedInternships}
                </div>
              </div>
            ))}
          </div>
